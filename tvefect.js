...
function tv () {
	...
	if(canvasCreado){
		...
		var canvas = document.querySelector("#tv"),
			...
			scanSize = 0,
			FPS = 50,
			scanSpeed = FPS * 5, // 15 seconds from top to bottom
			SAMPLE_COUNT = 10;
		
		window.onresize = function() {
			const {offsetHeight,offsetWidth} = document.querySelector('html');
			canvas.width = offsetWidth / scaleFactor;
			canvas.height = canvas.width / (offsetWidth /offsetHeight);
			scanSize = (offsetHeight / scaleFactor) / 3;
